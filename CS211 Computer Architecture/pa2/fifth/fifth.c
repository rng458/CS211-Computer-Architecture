...
void cycleDFS(struct node** graph, struct node* vertex, struct stack* stackHead, int* cycle) {
  ...
  while(eptr != 0) {
    if(graph[eptr->id]->visited) {
      if(inStack(stackHead, graph[eptr->id])) {
	*cycle = 1;
      }
      eptr = eptr->next;
    }
    else {
      cycleDFS(graph, graph[eptr->id], stackHead, cycle);
      eptr = eptr->next;
    }
  }

  struct stack* cleaner = sPop(&stackHead);
  free(cleaner);
  
}

int checkCycle(struct node** graph, int graphSize, struct node* vertex) {
  resetVisited(graph, graphSize);
  int cycle = 0;
  struct stack* cycleStack = malloc(sizeof(struct stack));
  cycleStack->vertex = 0;
  cycleStack->next = 0;
  cycleDFS(graph, graph[0], cycleStack, &cycle);
  if(cycle == 1) {
    return 1;
  }
  return 0;
...
void DAGSSP(struct node** graph, int graphSize, struct node* sourceVert) {
  ...
  for(int i = 0; i < graphSize; i++) {
    ...
    for(struct edge* ePtr = graph[vertU->id]->next; ePtr != 0; ePtr = ePtr->next) {
      if(graph[vertU->id]->distance != INT_MAX || ePtr->weight <= 0) {
...
int main (int argc, char* argv[argc+1]) {
  ...
  if(checkCycle(daGraph, graphSize, daGraph[0])) {
    printf("CYCLE\n");
